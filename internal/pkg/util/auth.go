...
// GetTraqUserID by c.Set("traqUserID", userName)
func GetTraqUserID(c echo.Context) (string, error) {
	userName, ok := c.Get("traqUserID").(string)
	if !ok {
		return "", fmt.Errorf("traqUserID not found in context")
	}
	return userName, nil
}
